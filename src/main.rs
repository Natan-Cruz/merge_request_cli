...
async fn main() -> ExitCode {
    ...
    let merge_request_number = MergeRequest::create_merge_request(&authorization.api_token, merge_request_body, &configurations.project_id).await;

    match merge_request_number {
        Ok(number) => {
            let success_message = format!("Link para MR: https://multiplier.jetbrains.space/p/srp/reviews/{}/timeline", number);
            loading_merge_request.success(success_message);
            loading_merge_request.end();
        },
        Err(error_message) => {
            loading_merge_request.fail(error_message);
            loading_merge_request.end();
